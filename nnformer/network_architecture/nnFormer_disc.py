...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm,tag=None):
        ...
        if tag==0:
            ...
            # keep stride-2 downsampling isotropic so the depth dimension stays aligned with the
            # subsequent transformer blocks (requires symmetric padding along the depth axis)
            self.reduction = nn.Conv3d(dim,dim*2,kernel_size=[3,3,3],stride=[2,2,2],padding=[1,1,1])
